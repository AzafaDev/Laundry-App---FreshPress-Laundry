...
export default function CustomerPaymentPage() {
  ...
  const handlePay = () => {
    ...
    createTransaction.mutate(orderId, {
      onSuccess: ({ token }) => {
        if (!window.snap) { setErrorMessage("Midtrans Snap belum siap, coba lagi sebentar."); return; }
      ...
      onError: (err: unknown) => {
        const message = (err as { response?: { data?: { message?: string } } })?.response?.data?.message ?? "Gagal membuat transaksi pembayaran.";
  ...
  return (
    <div className="min-h-screen bg-background text-on-surface">
      ...
      <main className="max-w-2xl mx-auto px-4 md:px-8 py-8 space-y-6">
        ...
        <div>
          <span className="inline-flex items-center rounded-full bg-primary/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.16em] text-primary mb-2">Pembayaran</span>
        ...
        {(isOrderLoading || isPaymentLoading) && (
          <div className="rounded-2xl border border-outline-variant bg-surface px-4 py-8 flex items-center justify-center gap-3 text-sm text-on-surface-variant">
            <Loader2 className="w-5 h-5 animate-spin text-primary" />Memuat detail pesanan...
          </div>
        )}
          ...
          <>
            <div className="rounded-2xl border border-outline-variant bg-surface p-4 md:p-6 shadow-sm space-y-3">
              <div className="flex items-center justify-between">
                ...
                {isExpiredOrFailed && (
                  <span className="inline-flex items-center gap-1.5 rounded-full bg-red-100 px-3 py-1 text-xs font-semibold text-red-700">
                    <XCircle className="w-3.5 h-3.5" />{payment?.status === "expired" ? "Kedaluwarsa" : "Gagal"}
                  </span>
                )}
              ...
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-medium text-on-surface">{order.pickup_schedule ? formatDateTime(order.pickup_schedule) : "-"}</span>
              </div>
