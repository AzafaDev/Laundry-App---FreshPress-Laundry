    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="flex items-center gap-3 text-on-surface-variant">
          ...
          <span className="text-sm font-medium">{t("notifications.preparingPage")}</span>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-background text-on-surface">
      ...
      <main className="max-w-3xl mx-auto px-4 md:px-8 py-8 space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <span className="inline-flex items-center rounded-full bg-primary/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.16em] text-primary mb-2">{t("notifications.badge")}</span>
            <h1 className="text-3xl font-bold text-on-surface">{t("notifications.title")}</h1>
            {unreadCount > 0 && <p className="text-sm text-on-surface-variant mt-1">{t("notifications.unreadCount", { count: unreadCount })}</p>}
          </div>
          {unreadCount > 0 && (
        ...
        {isLoading && (
          <div className="rounded-2xl border border-outline-variant bg-surface px-4 py-8 flex items-center justify-center gap-3 text-sm text-on-surface-variant">
            <Loader2 className="w-5 h-5 animate-spin text-primary" />{t("notifications.loading")}
          </div>
        )}
        {!isLoading && isError && (
          <div className="rounded-2xl border border-red-200 bg-red-50 px-4 py-5 text-sm text-red-700">
            <p className="font-semibold mb-2">{t("notifications.loadError")}</p>
            <button onClick={() => void refetch()} className="inline-flex items-center rounded-lg bg-red-600 px-3 py-2 text-xs font-semibold text-white hover:bg-red-700 transition-colors">{t("notifications.retry")}</button>
          </div>
        )}
        {!isLoading && !isError && notifications.length === 0 && (
          <div className="rounded-2xl border border-dashed border-outline-variant bg-surface px-6 py-12 text-center">
            ...
            <p className="font-semibold text-on-surface mb-1">{t("notifications.emptyTitle")}</p>
            <p className="text-sm text-on-surface-variant">{t("notifications.emptyDesc")}</p>
          </div>
        )}

        {!isLoading && !isError && total > 0 && (
          <p className="text-xs text-on-surface-variant">{t("notifications.showing", { shown: notifications.length, total })}</p>
        )}
        {!isLoading && !isError && notifications.length > 0 && (
        ...
        {!isLoading && !isError && totalPages > 1 && (
          <div className="flex items-center justify-center gap-2 pt-2">
            <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1} className="inline-flex items-center gap-1 rounded-xl border border-outline-variant px-3 py-2 text-sm font-medium text-on-surface disabled:opacity-40 hover:border-primary hover:text-primary transition-colors">
              <ChevronLeft className="w-4 h-4" />{t("notifications.previous")}
            </button>
            ...
            <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="inline-flex items-center gap-1 rounded-xl border border-outline-variant px-3 py-2 text-sm font-medium text-on-surface disabled:opacity-40 hover:border-primary hover:text-primary transition-colors">
              {t("notifications.next")}<ChevronRight className="w-4 h-4" />
            </button>
          </div>
        )}
      </main>
