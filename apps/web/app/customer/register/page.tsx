...
export default function CustomerRegisterPage() {
  ...
  const validate = (): boolean => {
    ...
    if (!form.phone.trim()) next.phone = t("auth.register.phoneRequired");
    else if (!/^[0-9+\-\s]{8,15}$/.test(form.phone.trim())) next.phone = t("auth.register.phoneInvalid");
    setErrors(next);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const msg = (err as { response?: { data?: { message?: string } } })?.response?.data?.message ?? t("auth.register.registerFailed");
    ...
    return (
      <main className="relative min-h-screen bg-background flex items-center justify-center p-4 overflow-hidden">
        ...
        <div className="w-full max-w-[520px] bg-white rounded-[2rem] shadow-2xl shadow-primary/5 border border-white p-8 md:p-10 flex flex-col items-center text-center gap-6">
          ...
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-primary tracking-tight">{t("auth.register.checkEmailTitle")}</h1>
            <p className="text-base text-on-surface-variant mt-2 px-4">
              {t("auth.register.checkEmailDesc", { email: form.email })}
            </p>
          </div>
          <div className="w-full bg-primary/5 border border-primary/20 rounded-2xl p-4 text-sm text-on-surface-variant">
            {t("auth.register.noEmailHint")}
          </div>
          <Link href="/verify" className="inline-flex items-center gap-2 bg-primary text-white py-3 px-8 rounded-2xl font-bold hover:brightness-110 transition-all shadow-lg shadow-primary/20">
            {t("auth.register.openVerifyPage")}<ArrowRight className="w-4 h-4" />
          </Link>
        </div>
      </main>
    );
      ...
      <div className="max-w-[1280px] w-full grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
        ...
        <div className="w-full max-w-[480px] mx-auto">
          ...
          <div className="bg-white border border-outline-variant rounded-2xl p-6 md:p-8 shadow-sm">
            <div className="mb-6">
              <h2 className="text-2xl font-bold text-on-surface">{t("auth.register.createFreeAccount")}</h2>
              <p className="text-on-surface-variant text-sm mt-1">{t("auth.register.subtitle")}</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {[
                { id: "name", label: t("auth.register.fullNameLabel"), icon: User, type: "text", placeholder: t("auth.register.fullNamePlaceholder"), field: "name" as const },
                { id: "email", label: t("auth.register.emailLabel"), icon: Mail, type: "email", placeholder: t("auth.register.emailPlaceholder"), field: "email" as const },
              ...
              <div className="group">
                <label htmlFor="phone" className="text-sm font-bold text-on-surface/80 ml-1 mb-1.5 block">{t("auth.register.phoneLabel")}</label>
                <div className="relative flex items-center">
                  <Phone className="absolute left-4 w-5 h-5 text-outline group-focus-within:text-primary transition-colors" />
                  <input id="phone" type="tel" placeholder={t("auth.register.phonePlaceholder")} value={form.phone} onChange={(e) => updateField("phone", e.target.value)} className="w-full pl-12 pr-4 py-3.5 rounded-2xl border-2 border-outline-variant bg-white focus:outline-none focus:border-primary transition-all text-base placeholder:text-outline-variant" />
                </div>
              ...
              <button type="submit" disabled={loading} suppressHydrationWarning className="w-full bg-primary text-on-primary font-bold py-3.5 rounded-2xl shadow-sm hover:brightness-110 active:scale-[0.98] transition-all flex items-center justify-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed">
                {loading ? <span className="inline-block animate-spin rounded-full h-5 w-5 border-b-2 border-white" /> : <>{t("auth.register.registerNow")} <ArrowRight className="w-5 h-5" /></>}
              </button>
            </form>

            <div className="relative my-5">
              <div className="absolute inset-0 flex items-center"><span className="w-full border-t border-outline-variant" /></div>
              <div className="relative flex justify-center text-xs"><span className="bg-white px-4 text-on-surface-variant uppercase tracking-wider">{t("auth.register.or")}</span></div>
            </div>
            ...
            <button type="button" onClick={handleGoogleRegister} suppressHydrationWarning className="w-full flex items-center justify-center gap-3 py-3 px-4 border border-outline-variant rounded-xl bg-white hover:bg-surface-container-low transition-colors active:scale-[0.99]">
              ...
              <span className="text-sm font-bold text-on-surface">{t("auth.register.registerWithGoogle")}</span>
            </button>

            <p className="text-center mt-5 text-sm text-on-surface-variant">
              {t("auth.register.haveAccount")} <Link href="/customer/login" className="text-primary font-bold hover:underline">{t("auth.register.loginHere")}</Link>
            </p>
          </div>
        </div>
      </div>
