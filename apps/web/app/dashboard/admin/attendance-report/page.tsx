...
export default function AttendanceReportPage() {
  ...
  const { data, isLoading } = useAttendanceReport(queryParams);
  ...
  const handleExportCSV = () => {
    ...
    const headers = ["Outlet", "Nama Karyawan", "Role", "Email", "Tanggal", "Check In", "Check Out", "Status"];
    const rows = records.map((att) => [
      att.outlet?.name ?? "-",
      att.user?.full_name ?? "-",
      ROLE_LABEL[att.user?.role ?? ""] ?? att.user?.role ?? "-",
      att.user?.email ?? "-",
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-base text-on-surface-variant">
          {isSuperAdmin
            ? "Lihat dan ekspor data absensi karyawan dari semua outlet."
            : "Lihat dan ekspor data absensi karyawan di outlet Anda."}
        </p>
      </div>

      {/* Filter Section */}
      <div className="bg-surface p-5 rounded-xl border border-outline-variant shadow-sm space-y-3">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {/* Filter by Outlet — super admin only */}
          {isSuperAdmin && (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-surface-container-low border-b border-outline-variant sticky top-0 z-10">
              <tr>
                {isSuperAdmin && <th className="px-6 py-4 text-sm font-bold">Outlet</th>}
            ...
            <tbody>
              ...
              {!isLoading && records.length === 0 && (
                <tr>
                  <td colSpan={isSuperAdmin ? 7 : 6} className="px-6 py-12 text-center text-on-surface-variant">
              ...
              {records.map((att) => (
                ...
                >
                  {isSuperAdmin && (
                    <td className="px-6 py-4 text-sm text-on-surface-variant">{att.outlet?.name ?? "-"}</td>
                  )}
                  ...
                  <td className="px-6 py-4">
                    <span
                      className={`px-2 py-1 rounded-full text-xs font-bold w-fit ${
                        STATUS_COLOR[att.status] ?? "bg-surface-container-high text-on-surface-variant"
                      }`}
                    >
                      {STATUS_LABEL[att.status] ?? att.status}
                    </span>
