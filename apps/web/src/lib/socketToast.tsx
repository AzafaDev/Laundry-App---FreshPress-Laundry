...
function ToastContent({ message, type, id }: { message: string; type: "success" | "error"; id: string }) {
  const isSuccess = type === "success";
  return (
    <div className={`flex items-start gap-3 px-4 py-3 rounded-2xl shadow-lg border max-w-xs w-full backdrop-blur-sm ${
      isSuccess
...
export function socketToast(message: string, type: "success" | "error" = "success") {
  toast.dismiss(SOCKET_TOAST_ID);
  toast.custom(
    (t) => <ToastContent message={message} type={type} id={t.id} />,
