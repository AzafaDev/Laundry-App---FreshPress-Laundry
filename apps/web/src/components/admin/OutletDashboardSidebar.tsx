...
// Outlet-admin scoped navigation. These point at the outlet dashboard subtree.
const NAV: NavItem[] = [
  { href: "/dashboard/outlet", label: "Dashboard", icon: LayoutDashboard },
  { href: "/dashboard/outlet/orders", label: "Orders", icon: Receipt },
  { href: "/dashboard/outlet/staff", label: "Staff", icon: Users },
  { href: "/dashboard/outlet/attendance", label: "Attendance", icon: CalendarCheck },
  { href: "/dashboard/outlet/reports", label: "Reports", icon: BarChart3 },
];
    ...
    <aside className="hidden lg:flex flex-col h-screen fixed left-0 top-0 w-72 bg-surface-container-low border-r border-outline-variant shadow-sm z-40">
      ...
      <div className="mx-4 p-4 bg-surface-container-high rounded-xl flex items-center gap-3 mb-2">
        <div className="w-10 h-10 rounded-full bg-primary text-on-primary flex items-center justify-center font-semibold flex-shrink-0">
          {user?.full_name?.slice(0, 1).toUpperCase() ?? "O"}
        </div>
        <div className="flex flex-col min-w-0">
          <span className="text-sm font-semibold text-on-surface truncate">
            {user?.full_name ?? "Outlet Admin"}
          </span>
      ...
      <nav className="flex-1 flex flex-col gap-0.5 px-3 overflow-y-auto">
        {NAV.map(({ href, label, icon: Icon }) => {
          const active =
            pathname === href ||
            (href !== "/dashboard/outlet" && pathname?.startsWith(href));
          return (
            <Link
              key={href}
              href={href}
