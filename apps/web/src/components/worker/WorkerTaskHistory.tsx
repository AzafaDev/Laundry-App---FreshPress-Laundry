...
function formatDuration(startedAt: string | null, completedAt: string | null) {
  ...
  const mins = Math.floor((new Date(completedAt).getTime() - new Date(startedAt).getTime()) / 60000);
  if (mins < 1) return "< 1 mnt";
  if (mins < 60) return `${mins} mnt`;
  return `${Math.floor(mins / 60)} jam ${mins % 60} mnt`;
}

const stationAccentBar: Record<StationType, string> = {
  washing: "bg-blue-400",
  ...
  if (tasks.length === 0) {
    return (
      <div className="text-center py-16 bg-surface-container-low rounded-2xl border border-dashed border-outline-variant">
        ...
        <p className="text-sm text-outline mt-1">Task station yang sudah kamu selesaikan akan muncul di sini</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      <AnimatePresence mode="wait">
        ...
        >
          {tasks.map((task) => {
            const config = stationConfig[task.station];
            ...
            return (
              <div key={task.id} className="bg-surface border border-outline-variant rounded-2xl overflow-hidden">
                <div className={`h-0.5 w-full ${stationAccentBar[task.station]}`} />
                <div className="p-4">
                  <div className="flex items-start gap-3">
                    <div className={`p-2 rounded-full shrink-0 ${config.iconClass}`}>
                      <Icon className="w-5 h-5" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2 mb-1">
                        <span className={`text-xs font-bold uppercase tracking-widest px-2 py-0.5 rounded-full ${config.badgeClass}`}>
                          {config.title.replace("Stasiun ", "")}
                        </span>
                  ...
                  <div className="flex items-center justify-between mt-3 pt-3 border-t border-outline-variant/60 gap-2 flex-wrap">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {duration && (
                        <span className="text-xs font-medium bg-surface-container-low text-on-surface-variant px-2 py-0.5 rounded-full flex items-center gap-1">
